...
Pinned from [larksuite/cli `lark-slides`](https://github.com/larksuite/cli/tree/cb5a3d704379552dc61e37898e5f74798783190d/skills/lark-slides) at `cb5a3d704379552dc61e37898e5f74798783190d` (MIT). Listing: [skills.sh](https://www.skills.sh/larksuite/cli/lark-slides). Requires `lark-cli` (`npm i -g @larksuite/cli`). Shared auth: `skills/lark-shared/SKILL.md` (`../lark-shared/SKILL.md`). Provenance: `skills/lark-slides/SOURCE.txt`.

**In this repo:** `skills/lark-slides/SKILL.md` plus `references/` and `scripts/`. Scoped rule: `rules/lark-slides.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/lark-slides/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/lark-slides`.

Do not apply to generic PPTX export (use Anthropic `pptx`), web UI, or backend work. Do not run `lark-cli auth login`, write, delete, or upload unless the user asked. Do not print secrets. Confirm high-risk writes. Prefer `--dry-run` first.
