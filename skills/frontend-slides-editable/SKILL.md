...
Pinned from [archlizheng/frontend-slides-editable](https://github.com/archlizheng/frontend-slides-editable/tree/e5663e6a0bdc2c2a673198bab3fb61036a7f20ee) at `e5663e6a0bdc2c2a673198bab3fb61036a7f20ee` (MIT). Listing: [skills.sh](https://www.skills.sh/archlizheng/frontend-slides-editable/frontend-slides-editable). Official CLI: `npx skills add https://github.com/archlizheng/frontend-slides-editable --skill frontend-slides-editable`. Provenance: `SOURCE.txt`.

**In this repo:** `skills/frontend-slides-editable/SKILL.md`. Scoped rule: `rules/frontend-slides-editable.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/frontend-slides-editable/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/frontend-slides-editable`. Distinct from `/slides` (Chart.js ckmslides) and `/lark-slides` (Feishu XML).

Do not install `puppeteer-core` or scrape stock photos unless the user asked. PPTX/PDF extract scripts need `python-pptx` / `pymupdf` only when converting those files. `docs/preset-previews/` PNGs are not vendored; use `STYLE_PRESETS.md` and `examples/` instead. Discovery gate still applies: invoking this skill does not waive Phase 1.
