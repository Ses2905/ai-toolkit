...
Pinned from [kozz36/frontend-designer-skill `frontend-designer`](https://github.com/kozz36/frontend-designer-skill/tree/a1ca0df80260fc93dca89501802972eae49c29d4/skills/frontend-designer) at `a1ca0df80260fc93dca89501802972eae49c29d4` (Apache-2.0). Official CLI: `npx skills add https://github.com/kozz36/frontend-designer-skill --skill frontend-designer`. Provenance: `SOURCE.txt`.

**In this repo:** `skills/frontend-designer/SKILL.md`. Scoped rule: `rules/frontend-designer.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/frontend-designer/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/frontend-designer`. Distinct from `/frontend-design` (Anthropic visual identity), `/web-design-guidelines` (heuristic UI review), and `/design-system` (this kit's token CSVs). `frontend-designer-lite` is not vendored; use this full skill with `references/`.
