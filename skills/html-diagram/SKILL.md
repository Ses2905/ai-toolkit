...
Pinned from [plannotator/effective-html `html-diagram`](https://github.com/plannotator/effective-html/tree/2ac1dfecb0f2474e75260cb6d3c9b9d6d9b5062e/skills/html-diagram) at `2ac1dfecb0f2474e75260cb6d3c9b9d6d9b5062e` (MIT). Listing: [skills.sh](https://www.skills.sh/plannotator/effective-html/html-diagram). Official CLI: `npx skills add https://github.com/plannotator/effective-html --skill html-diagram`. Visual register: `../design-artifact/SKILL.md`. Provenance: `SOURCE.txt`.

**In this repo:** `skills/html-diagram/SKILL.md`. Scoped rule: `rules/html-diagram.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/html-diagram/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/html-diagram`. Distinct from `/canvas-design` (static poster/PDF) and `/slides` (pitch decks).

Deliver one self-contained HTML file. Do not install `@plannotator/tot` or publish to tot.page unless the user asked. Do not use Google Fonts CDNs; embed type as data URIs if needed.
