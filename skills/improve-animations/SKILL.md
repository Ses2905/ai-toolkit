...
Pinned from [emilkowalski/skills `improve-animations`](https://github.com/emilkowalski/skills/tree/85e8e2363b713506e1d5b6e07a0eb2da66be1bc3/skills/improve-animations) at `85e8e2363b713506e1d5b6e07a0eb2da66be1bc3` (MIT). Listing: [skills.sh](https://www.skills.sh/emilkowalski/skills/improve-animations). Official CLI: `npx skills add https://github.com/emilkowalski/skills --skill improve-animations`. Rule catalog: [AUDIT.md](AUDIT.md). Plan format: [PLAN-TEMPLATE.md](PLAN-TEMPLATE.md). Provenance: `SOURCE.txt`.

**In this repo:** `skills/improve-animations/SKILL.md`. Scoped rule: `rules/improve-animations.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/improve-animations/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/improve-animations`. Distinct from `/emil-design-eng` (authoring), `/find-animation-opportunities` (missing motion), `/review-animations` (single-diff review), and `/apple-design` (authoring Apple-style motion). Read-only on source — write plans under `plans/` (or `animation-plans/`); do not implement unless the user asked `execute`.
