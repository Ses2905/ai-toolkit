...
**In this repo:** `skills/improve-animations/SKILL.md`. Scoped rule: `rules/improve-animations.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/improve-animations/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/improve-animations`. Distinct from `/find-animation-opportunities` (missing motion), `/review-animations` (single-diff review), and `/apple-design` (authoring Apple-style motion). Read-only on source — write plans under `plans/` (or `animation-plans/`); do not implement unless the user asked `execute`.
