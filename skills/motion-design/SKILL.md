...
Pinned from [LottieFiles/motion-design-skill `motion-design`](https://github.com/LottieFiles/motion-design-skill/tree/f9a8a041b85185ee4881b3471d3415e939aac772/skills/motion-design) at `f9a8a041b85185ee4881b3471d3415e939aac772` (MIT). Official CLI: `npx skills add https://github.com/LottieFiles/motion-design-skill --skill motion-design`. Provenance: `SOURCE.txt`.

**In this repo:** `skills/motion-design/SKILL.md`. Scoped rule: `rules/motion-design.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/motion-design/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/motion-design`. Distinct from `/hyperframes-animation` (HyperFrames/GSAP adapters), `/remotion-best-practices` (Remotion video), `/review-animations` (motion diffs), `/improve-animations` (audit/plans), and `/emil-design-eng` (UI craft bar). Runtime-agnostic principles; pick CSS, GSAP, Framer Motion, Lottie, or Spring after the three pillars.
