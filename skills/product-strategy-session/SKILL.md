...
Pinned from [deanpeters/product-manager-skills `product-strategy-session`](https://github.com/deanpeters/product-manager-skills/tree/1b5a524ebb95e9497fa3f25002d8b8ec528d4444/skills/product-strategy-session) at `1b5a524ebb95e9497fa3f25002d8b8ec528d4444` (CC BY-NC-SA 4.0). Listing: [skills.sh](https://www.skills.sh/deanpeters/product-manager-skills/product-strategy-session). Official CLI: `npx skills add https://github.com/deanpeters/product-manager-skills --skill product-strategy-session`. Facilitation protocol: `../workshop-facilitation/SKILL.md`. Provenance: `SOURCE.txt`.

**In this repo:** `skills/product-strategy-session/SKILL.md`. Scoped rule: `rules/product-strategy-session.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/product-strategy-session/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/product-strategy-session`. Upstream `skills/<name>/SKILL.md` paths are siblings here (`../<name>/SKILL.md`). License is **non-commercial**; do not treat this pack as MIT.

Do not skip phases to “save turns.” Anything in the invocation counts as answers already given — do not re-ask. Distinct from `/plan-the-work` (engineering implementation planning).
