...
Pinned from [lewislulu/html-ppt-skill](https://github.com/lewislulu/html-ppt-skill/tree/fd1629067909ff55b36b905476de4e5f26062a1f) at `fd1629067909ff55b36b905476de4e5f26062a1f` (MIT). Listing: [skills.sh](https://www.skills.sh/lewislulu/html-ppt-skill/html-ppt). Official CLI: `npx skills add https://github.com/lewislulu/html-ppt-skill --skill html-ppt`. Provenance: `SOURCE.txt`.

**In this repo:** `skills/html-ppt/SKILL.md`. Scoped rule: `rules/html-ppt.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/html-ppt/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/html-ppt`. Distinct from `/slides` (Chart.js ckmslides), `/frontend-slides-editable` (single-file editor), and `/lark-slides` (Feishu XML).

Do not run `scripts/render.sh` (headless Chrome) or download webfonts unless the user asked. `scripts/verify-output/` PNGs and `docs/readme` marketing images are not vendored; use `templates/` and `references/` instead. Ask the three authoring questions before writing slides.
