...
Pinned from [heygen-com/hyperframes `hyperframes-animation`](https://github.com/heygen-com/hyperframes/tree/d6fe66707f74467bd7193839cb4b5cb3f1e8bb06/skills/hyperframes-animation) at `d6fe66707f74467bd7193839cb4b5cb3f1e8bb06` (Apache-2.0, v0.8.43). Listing: [skills.sh](https://www.skills.sh/heygen-com/hyperframes/hyperframes-animation). Official CLI: `npx skills add https://github.com/heygen-com/hyperframes --skill hyperframes-animation`. Contract: `../hyperframes-core/SKILL.md`. Provenance: `SOURCE.txt`.

**In this repo:** `skills/hyperframes-animation/SKILL.md`. Scoped rule: `rules/hyperframes-animation.mdc` (`alwaysApply: false`). User install: `~/.cursor/skills/hyperframes-animation/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`. Invoke with `/hyperframes-animation`. Distinct from `/react-bits` and `/apple-design`.

Do not run `npx hyperframes` render/preview, install `@hyperframes/producer`, or call paid HeyGen APIs unless the user asked. Script path from this plugin: `skills/hyperframes-animation/scripts/animation-map.mjs` (user copy: `~/.cursor/skills/hyperframes-animation/scripts/animation-map.mjs`).
