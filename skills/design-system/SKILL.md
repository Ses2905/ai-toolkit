...
Pinned with `slides` (ckmslides) from [nextlevelbuilder/ui-ux-pro-max-skill `design-system`](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill/tree/15de38fb70bc80ae9276fa7703b48ae861a672e6/cli/assets/skills/design-system) at `15de38fb70bc80ae9276fa7703b48ae861a672e6` (MIT). Required sibling of `skills/slides`. Provenance: `skills/design-system/SOURCE.txt`. Optional upstream siblings `brand` and `ui-styling` are not pinned here.

**In this repo:** `skills/design-system/SKILL.md` plus `data/`, `scripts/`, `references/`, `templates/`. User install: `~/.cursor/skills/design-system/` via `./scripts/install-local.sh` then `./scripts/sync-user-skills.sh`.

Do not scrape stock-photo sites or call paid APIs unless the user asked. Keep the working directory at the project root; scripts resolve `assets/design-tokens.json` from cwd.
